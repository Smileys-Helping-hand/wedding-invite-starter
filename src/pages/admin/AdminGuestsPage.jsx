...
const AdminGuestsPage = ({
  ...
  const handleSubmit = (event) => {
    ...
    const code = generateInviteCode
      ...
      : desiredCode ||
        `${names[0].slice(0, 4).toUpperCase()}${String(Date.now()).slice(-4)}`;

    const payload = {
      code,
      guestNames: names,
      householdId:
        form.householdId?.trim() || (getNextHouseholdId ? getNextHouseholdId() : ''),
      contact: form.contact.trim(),
      householdCount: Math.max(
        Number(form.householdCount) || names.length || 1,
        names.length || 1
      ),
  ...
  return (
    <div className="admin-guests">
      ...
      <header className="admin-guests__header">
        <div>
          ...
          <p className="page-subtitle">
            Gracefully manage each household, update RSVPs, and share invite links in
            moments.
          </p>
        </div>
        <div className="admin-guests__header-actions">
          <Button
            variant="ghost"
            size="md"
            onClick={() => onExportCsv?.(filteredEntries)}
          >
            Export CSV
          </Button>
      ...
      <form className="admin-guests__quick" onSubmit={handleQuickAdd}>
        ...
        <TextInput
          label="Partner name"
          value={quickForm.partner}
          onChange={(event) =>
            setQuickForm((prev) => ({ ...prev, partner: event.target.value }))
          }
        />
        <TextInput
          ...
          onChange={(event) =>
            setQuickForm((prev) => ({ ...prev, contact: event.target.value }))
          }
      ...
      <div className="admin-guests__toolbar">
        ...
        />
        <div
          className="admin-guests__filters"
          role="tablist"
        ...
        {filteredEntries.map((guest) => (
          <div key={guest.code} className="admin-guests__row" role="row">
            <span role="cell">
              ...
              <small className="muted">
                Household {guest.householdId ?? '—'} • {guest.householdCount ?? 1} invited
              </small>
              <small className="muted">Role: {guest.role ?? 'guest'}</small>
            </span>
            <span role="cell">
              <span className={`status-pill status-pill--${guest.rsvpStatus}`}>
                {STATUS_LABELS[guest.rsvpStatus]}
              </span>
              <div className="status-actions">
                <button
                  type="button"
                  onClick={() => handleStatusUpdate(guest, RSVP_STATUSES.confirmed)}
                >
                  Confirm
                </button>
                <button
                  type="button"
                  onClick={() => handleStatusUpdate(guest, RSVP_STATUSES.pending)}
                >
                  Pending
                </button>
                <button
                  type="button"
                  onClick={() => handleStatusUpdate(guest, RSVP_STATUSES.declined)}
                >
                  Regret
                </button>
              </div>
            ...
            <form className="admin-guests__form" onSubmit={handleSubmit}>
              <div className="form-grid">
                ...
                <TextInput
                  ...
                  onChange={(event) =>
                    handleChange('householdId', event.target.value.toUpperCase())
                  }
                />
                <TextInput
