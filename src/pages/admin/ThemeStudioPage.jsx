...
const ThemeStudioPage = ({ entries = [] }) => {
  ...
  useEffect(() => {
    setStatus((previous) => {
      ...
      return previous === 'Launch Mode enabled. Preview only.'
        ? 'Editing unlocked.'
        : previous;
    });
  }, [launchMode]);
  ...
  const handleReset = () => {
    ...
    const confirmReset = window.confirm(
      'Reset theme to the default look? This cannot be undone.'
    );
  ...
  const handleFoilIntensity = (value) => {
    ...
    updateTheme({
      toggles: { goldFoilIntensity: intensity, foilSheenIntensity: intensity },
    });
  };
    ...
    <section className="studio-panel">
      ...
      <div className="studio-field-grid">
        ...
        <TextInput
          ...
          onChange={(event) =>
            handleTextChange('bismillahTranslation', event.target.value)
          }
          disabled={launchMode}
        />
        <TextInput
  ...
  const renderMediaTab = () => (
    <section className="studio-panel">
      ...
      <p className="studio-hint">
        Drag &amp; replace media or upload above. Larger files will take a moment to
        shimmer in.
      </p>
      <div className="upload-grid">
        ...
        <label className="upload-field">
          ...
          <input
            ...
            onChange={(event) =>
              handleAssetUpload('sparklesVideo', event.target.files?.[0])
            }
            disabled={launchMode}
          />
        </label>
        ...
        <p className="upload-tip" role="note">
          Drag &amp; replace media or upload above.
        </p>
      </div>
    </section>
  );
      ...
      <div className="studio-field-grid">
        <label className="studio-field">
          ...
          <select
            ...
            onChange={(event) =>
              handleToggleChange('animationIntensity', event.target.value)
            }
            disabled={launchMode}
          >
        ...
        <label className="switch-field">
          <input
            ...
            onChange={(event) =>
              handleToggleChange('nasheedAutoplay', event.target.checked)
            }
            disabled={launchMode}
          />
      ...
      <div className="theme-studio__header">
        <div>
          ...
          <p className="page-subtitle">
            Adjust palettes, typography, media, and motion for {entries.length} household
            {entries.length === 1 ? '' : 's'} currently invited. Save drafts locally or
            publish for every guest.
          </p>
          {launchMode && (
            <p className="launch-mode-alert">
              Launch Mode is enabled. Disable it to continue editing.
            </p>
          )}
        </div>
        ...
        <section className="studio-panel preview-panel">
          ...
          <div className="preview-frame" data-device={previewDevice}>
            <div
              ...
              data-shape={toggles.cardEdgeStyle ?? 'rounded'}
            >
              ...
              <div className="preview-envelope">
                <img
                  src={assets.envelope ?? defaultAssets.envelope}
                  alt="Envelope"
                  className="preview-envelope__paper"
                />
                <img
                  src={
                    (theme?.assets?.waxSeals ?? waxSeals)[
                      theme?.assets?.waxSealVariant ?? selectedWax
                    ]
                  }
                  alt="Wax seal"
              ...
              <div className="preview-invite-card">
                <img
                  src={assets.inviteCard ?? defaultAssets.inviteCard}
                  alt="Invite card"
                />
              </div>
            </div>
