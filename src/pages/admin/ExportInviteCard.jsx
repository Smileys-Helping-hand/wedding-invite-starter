...
const ExportInviteCard = ({ guests = [] }) => {
  ...
  const assetBackground =
    theme?.assets?.inviteCard ?? getAssetPath('cardBackground', theme?.assets);

  const coupleNames = useMemo(
    () => theme?.texts?.coupleNames ?? 'Razia & Abduraziq',
    [theme?.texts?.coupleNames]
  );
  const blessing = theme?.texts?.blessing ?? 'بِسْمِ اللّٰهِ الرَّحْمٰنِ الرَّحِيْمِ';
  const scheduleGregorian =
    theme?.text?.scheduleGregorian ??
    theme?.texts?.scheduleGregorian ??
    'Tuesday, 16 December 2025';
  const scheduleVenue =
    theme?.text?.scheduleVenue ??
    theme?.texts?.scheduleVenue ??
    EVENT_VENUE.addressLine1 ??
    'Legacy Events, Schaapkraal Road, Ottery, Cape Town';
  const scheduleTime =
    theme?.text?.scheduleTime ??
    theme?.texts?.scheduleTime ??
  ...
  const handleExport = async () => {
    ...
    setIsExporting(true);
    try {
      await downloadCanvas(squareRef.current, {
        width: 1080,
        height: 1080,
        fileName: 'invite-square.png',
      });
      await downloadCanvas(storyRef.current, {
        width: 1080,
        height: 1920,
        fileName: 'invite-story.png',
      });
    } catch (err) {
  ...
  return (
    <div className="admin-export">
      <header className="admin-export__header">
        <div>
          ...
          <p className="page-subtitle">
            Generate WhatsApp-ready graphics using your current theme styling for{' '}
            {guests.length} household{guests.length === 1 ? '' : 's'}.
          </p>
        </div>
      ...
      <div className="admin-export__previews">
        <div className="export-preview">
          ...
          <div className="export-card export-card--square" ref={squareRef}>
            <div
              className="export-card__panel"
              style={{ backgroundImage: `url(${assetBackground})` }}
            >
      ...
      <p className="admin-export__hint">
        PNG files will download to your device and can be shared on WhatsApp or Instagram.
      </p>
    </div>
  );
};
