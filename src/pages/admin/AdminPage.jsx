...
const AdminPage = () => {
  ...
  const [entries, setEntries] = useState(() => {
    ...
    if (storedGuest && !localGuests.some((guest) => guest.code === storedGuest.code)) {
      return [...localGuests, storedGuest];
    }
    return localGuests;
  });
  ...
  const stats = useMemo(() => {
    ...
    const guestsAttending = entries.reduce((acc, guest) => {
      if (guest.rsvpStatus === RSVP_STATUSES.confirmed) {
        return acc + (guest.household ?? 1) + (guest.additionalGuests ?? 0);
      }
      return acc;
    }, 0);
  ...
  return (
    <div className="page-panel admin-dashboard">
