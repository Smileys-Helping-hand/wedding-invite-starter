...
import MemoryWallModeration from './MemoryWallModeration.jsx';
...
const computeInviteCode = (
  ...
  const prefix = (
    baseLetters.length >= 4 ? baseLetters.slice(0, 4) : baseLetters.padEnd(4, 'A')
  ).toUpperCase();
...
const buildGuestPayload = (payload, options) => {
  ...
  return normaliseGuest({
    ...
    guestNames,
    householdId,
    householdCount: Math.max(
      Number(payload.householdCount) || guestNames.length || 1,
      guestNames.length || 1
    ),
...
const AdminPage = () => {
  ...
  const ADMIN_PASS = (import.meta.env.VITE_ADMIN_PASSWORD ?? '2025').toString();
  ...
  const [isAuthenticated, setIsAuthenticated] = useState(() => {
    ...
    try {
      ...
      return decodeSessionToken(stored) === ADMIN_PASS;
    } catch (err) {
      return false;
    }
  });
  const [entries, setEntries] = useState(() => {
    ...
    const combined =
      storedGuest && !localGuests.some((guest) => guest.code === storedGuest.code)
  ...
  const subscribeToGuests = firebase?.subscribeToGuests;

  useEffect(() => {
    if (!subscribeToGuests) return undefined;
  ...
  const stats = useMemo(() => {
    ...
    const confirmed = entries.filter(
      (guest) => guest.rsvpStatus === RSVP_STATUSES.confirmed
    ).length;
    const pending = entries.filter(
      (guest) => guest.rsvpStatus === RSVP_STATUSES.pending
    ).length;
    const declined = entries.filter(
      (guest) => guest.rsvpStatus === RSVP_STATUSES.declined
    ).length;
  ...
  const updateEntries = useCallback(
    (mutator, undoDescription) => {
      setEntries((prev) => {
        const next = mutator(prev).sort((a, b) =>
          a.primaryGuest.localeCompare(b.primaryGuest)
        );
        persistEntries(next);
        if (undoDescription) {
          const snapshot = prev.map((guest) => ({
            ...guest,
  ...
  const handleUpdateGuest = useCallback(
    (existingCode, payload) => {
      ...
      logAction('guest:update', {
        from: existingCode,
        to: guest.code,
        name: guest.primaryGuest,
      });
      firebase?.addGuest?.(guest.code, guest).catch(() => {});
  ...
  const handleDeleteGuest = useCallback(
    (code) => {
      ...
      if (!current) return;
      updateEntries(
        (prev) => prev.filter((guest) => guest.code !== code),
        `Removed ${current.primaryGuest}`
      );
  ...
  const handleStatusChange = useCallback(
    (code, status) => {
      ...
      firebase
        ?.saveRSVP?.(code, { rsvpStatus: status, lastUpdated: new Date().toISOString() })
        .catch(() => {});
  ...
  const getNextHouseholdId = useCallback(
    ...
    return (
      <div className="page-panel admin-login">
        ...
        >
          ...
          <p>
            Enter the passcode to view RSVP insights, manage guests, and curate the theme
            studio.
          </p>
          <TextInput
    ...
    <div className="admin-shell">
      ...
      <div className="admin-content">
        <Routes>
            ...
            element={
              <div className="admin-dashboard">
                <header className="admin-dashboard__header">
                  <div>
                    ...
                    <p className="page-subtitle">
                      Live RSVP status with quick insight into your honoured guests.
                    </p>
                  </div>
                ...
                <section className="admin-dashboard__health">
                  ...
                  <div className="admin-dashboard__health-note">
                    <h3>Unified monitor</h3>
                    <p>
                      Track the most recent heartbeat checks and open the live monitor to
                      review latency, uptime, and CDN health in one glance.
                    </p>
                    <Button
                      variant="ghost"
                      size="sm"
                ...
                <footer className="admin-dashboard__footer">
                  System Status &rarr;{' '}
                  <a href="/monitor" className="admin-dashboard__footer-link">
                    /monitor
                  </a>
                </footer>
              </div>
          ...
          <Route path="memories" element={<MemoryWallModeration />} />
          <Route
            path="import"
            element={<GuestSpreadsheetImporter existingGuests={entries} />}
          />
