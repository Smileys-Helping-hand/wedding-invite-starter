...
const EnvelopeStage = ({ onOpened, onRevealEnd, sealVariant = 'default' }) => {
  ...
  const [envelopeSrc, setEnvelopeSrc] = useState(
  ...
  const [waxSrc, setWaxSrc] = useState(() =>
    getWaxSeal(resolvedVariant, theme?.assets?.waxSeals ?? theme?.assets)
  );
  ...
  const foilIntensity = Math.min(
    Math.max(
      theme?.toggles?.foilSheenIntensity ?? theme?.toggles?.goldFoilIntensity ?? 0.6,
      0
    ),
    1
  );
  ...
  const announceReveal = useCallback(() => {
    ...
    window.requestAnimationFrame(() => {
      ...
      if (typeof window !== 'undefined' && window.CustomEvent) {
        window.dispatchEvent(new window.CustomEvent('envelope:reveal-end'));
      }
    });
  }, [onOpened, onRevealEnd]);
  ...
  const handleClick = useCallback(() => {
    ...
    try {
      window.localStorage.setItem('hasOpenedEnvelope', 'true');
    } catch (storageError) {
      if (import.meta.env.DEV) {
        console.warn('Unable to persist envelope state', storageError);
      }
    }
  }, [isMelting, isOpen, isPreMelt, startAudio]);
  ...
  useEffect(() => {
    ...
    try {
      alreadyOpened = window.localStorage.getItem('hasOpenedEnvelope') === 'true';
    } catch (storageError) {
      if (import.meta.env.DEV) {
        console.warn('Unable to read envelope state', storageError);
      }
    }
    ...
    >
      ...
      <div className="envelope-stage__stack">
        <motion.div
          ...
          initial={{ opacity: 0, scale: 1.04, y: 52 }}
          animate={
            isOpen
              ? { opacity: 0.25, scale: 0.95, y: 28 }
              : { opacity: 1, scale: [1.04, 1.02, 1], y: [52, 12, 0] }
          }
          transition={{
